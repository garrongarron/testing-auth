import { singOut } from "./Auth.js"
import fb from './Facebook.js'
import gg from './Google.js'


let showAuthInterface = (nav, isLogged) => {
    let btnToSingIn = `
        <button class="gg">
...
let authentication = (isLogged) => {
    let nodeElement = document.querySelector('nav')
    showAuthInterface(nodeElement, isLogged)
    eventHandlers(isLogged)
}
let hideOption = (selector) => {
